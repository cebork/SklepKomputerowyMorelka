            ...
            <div class="row">
                ...
                <div class="col-8 ">
                    ...
                    <?php
                        ...
                        $query_for_dostawy = "SELECT * FROM dane_personalne d JOIN users u ON d.user_id = u.user_id JOIN zamowienia z ON u.user_id = z.user_id JOIN statusy s ON z.status_id = s.status_id WHERE z.zamowienia_id = {$_GET["zam_id"]}";
                        $result_for_dostawy = $connection->query($query_for_dostawy);
                        $row_for_dostawy = $result_for_dostawy->fetch_assoc();
                        echo "<p>Status: <b>{$row_for_dostawy["status"]} </b>";
                        echo "<h2>Dane dostawy</h2>";
                        echo "<p>Imię: <b>{$row_for_dostawy["imie"]} </b>";
                        echo "<p>Nazwisko: <b>{$row_for_dostawy["nazwisko"]} </b>";
                        echo "<p>E-mail: <b>{$row_for_dostawy["email"]} </b>";
                        echo "<p>Telefon: <b>{$row_for_dostawy["telefon"]} </b>";
                        echo "<p>Miejscowość: <b>{$row_for_dostawy["miejscowosc"]} </b>";
                        echo "<p>Kod pocztowy: <b>{$row_for_dostawy["kod_pocztowy"]} </b>";
                        echo "<p>Ulica: <b>{$row_for_dostawy["ulica"]} </b>";
                        echo "<p>Numer domu: <b>{$row_for_dostawy["nr_domu"]} </b>";
                        echo "<h3>Zmiana statusu</h3>"; 
                        echo "<form method='POST' action='../CRUD/changeStatus.php'>";
                            echo "<select name='zamowienia_statusy' style='width: 200px;' id='cat' class='choose form-select' aria-label='Default select example'>";
                                $query_statusy = "SELECT * FROM statusy";
                                $result_statusy = $connection->query($query_statusy);
                                $row_statusy = $result_statusy->fetch_assoc();
                                while($row_statusy = $result_statusy->fetch_assoc()){
                                    echo "<option value='{$row_statusy["status_id"]}'>{$row_statusy["status"]}</option>";
                                }
                            echo "</select>";
                            echo "<input type='hidden' name='zam_id' value='{$_GET["zam_id"]}'>";
                            echo "<button type='submit' name='aktualizuj' class='btn btn-primary'>Aktualizuj</button>";
                        echo "</form>";
                    ?>
                    
                    <p  class="justify-content-center"><a class="d-flex justify-content-center" href="../content/orderUpdate.php">Wróć</a></p>
                </div>
                <div class="col-2 ">
                </div> 
            </div> 
