                ...
                <div class="col-8 ">
                    <table class="table table-striped">
                        <thead>
                            ...
                            <th scope="col">Punkty Lojalnościowe</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $query_for_ranking = "SELECT imie, nazwisko, PL FROM users u, dane_personalne d WHERE u.user_id = d.user_id ORDER BY PL DESC LIMIT 20";
                            $result_query_for_ranking = $connection->query($query_for_ranking);
                            $counter_for_ranking = 1;
                            if($result_query_for_ranking->num_rows > 0){
                                while($row_for_ranking = $result_query_for_ranking->fetch_assoc()){
                                    echo "<tr>";
                                    echo "<th scope='row'>{$counter_for_ranking}</th>";
                                    echo "<td>{$row_for_ranking['imie']}</th>";  
                                    echo "<td>{$row_for_ranking['nazwisko']}</th>";  
                                    echo "<td>{$row_for_ranking['PL']}</th>";   
                                    echo "</tr>";
                                    $counter_for_ranking++;
                                }
                            }
